  ...
  if (ball && cam && cv && vid && !reduced) {
    ...
    function tick() {
      ...
      /* נקודת הנחיתה נגזרת מהמלבן של הפינים, לא מגובה החלון.
         ...
         PIN_BASE נמדד מתוך הפריים הראשון של strike-pins.mp4:
         הפינים ניצבים בין 18% ל-80% מגובה הפריים, כלומר הבסיס
         שלהם - הגובה שבו כדור אמיתי פוגע - יושב ב-80%.
         מרכז הכדור מונח שם, כך שהוא נראה מונח על המסלול בגובה
         הפינים ולא מרחף מעליהם. להורדה או העלאה - זה המספר. */
      var PIN_BASE = 0.8;
      var camBox = cam.getBoundingClientRect();
      var maxY = camBox.top + camBox.height * PIN_BASE - ball.offsetHeight / 2;
